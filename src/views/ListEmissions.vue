...
export default {
  ...
  methods: {
    findOneById(source, id) {
      return source.find(el => el.id === id);
    },

    async sourceRows({
      worksites, facilities, facilityLocations, emissionSources, releaseSources,
    }, calc, pollutant) {
      const releaseSource = await this.findOneById(releaseSources, calc.releaseSourceId);
      const emissionSource = await this.findOneById(emissionSources, releaseSource.emissionSourceId);
      const facilityLocation = await this.findOneById(facilityLocations, releaseSource.facilityLocationId);
      const facility = await this.findOneById(facilities, releaseSource.facilityId);
      const worksite = await this.findOneById(worksites, releaseSource.worksiteId);
      this.company_emissions.push({
        release_source: releaseSource.releaseSourceName,
        emission_source: emissionSource.emissionSourceName,
        facility_location: facilityLocation.facilityLocationName,
        facility: facility.facilityName,
        work_site: worksite.workSiteName,
        polCode: pollutant.code,
        polName: pollutant.name,
        gsecemissions: pollutant.gseccoef,
        tyearemissions: pollutant.tyearcoef,
      });
    },
  },

  async created() {
    const worksites = await api.getResource('worksites');
    const facilities = await api.getResource('facilities');
    const facilityLocations = await api.getResource('facilityLocations');
    const emissionSources = await api.getResource('emissionSources');
    const releaseSources = await api.getResource('releaseSources');
    const actualCalculations = await api.getResource('releaseSourceCalculations?calcType=actual');
    const sources = {
      worksites, facilities, facilityLocations, emissionSources, releaseSources, actualCalculations,
    };
    actualCalculations.map(calc => calc.pollutants.map(pollutant => this.sourceRows(sources, calc, pollutant)));
  },
};
</script>
