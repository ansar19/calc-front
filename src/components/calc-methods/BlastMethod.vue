<template>
  ...
  <div class="container mt-4">
    ...
    <h5>Расчет выбросов ЗВ в атмосферу выполнен согласно "Методике расчета выбросов загрязняющих веществ в атмосферу от
    ...
    <form class="mb-3">
      ...
      <div class="form-group">
        ...
        <input id="blastedmaterial" v-model.number="blastedmaterial" type="text" class="form-control">
      </div>
      ...
      <div class="form-group">
        ...
        <span id="gasSuppressionHelpBlock" class="form-text text-muted">*При применении гидрозабойки эффективность
          подавление оксидов азота составляет =0.35-0.5</span>
      </div>

      <div class="form-group">
        <label for="noPart">Удельное выделение i-того загрязняющего вещества из взорванной горной породы - NOx</label>
      ...
      <div class="form-group">
        <label for="coPart">Удельное выделение i-того загрязняющего вещества из взорванной горной породы - CO</label>
        <div>
      ...
      <div class="form-group">
        <label for="sigleBlastQty">Количество взорванного взрывчатого вещества за один массовый взрыв, т/взрыв</label>
        <input id="sigleBlastQty" v-model.number="singleBlastQty" type="text" class="form-control">
      </div>

      <div class="form-group">
        <label for="sigleBlastRock">Максимальный объем взорванной горной породы за один массовый взрыв, м3/взрыв</label>
        <input id="sigleBlastRock" v-model.number="sigleBlastRock" type="text" class="form-control">
      </div>

      <div @click.prevent="seen = !seen" class="control">
...
export default {
  ...
  computed: {
    ...
    m0301() {
      return (this.m0301 =
              (((this.specificNOx * this.blastedmaterial * (1 - this.gasSupression)) + (this.noPart * this.blastedmaterial)) * 0.8).toFixed(7));
    },
    g0304() {
      return (this.g0304 =
        ((this.specificNOx *
    ...
    gsecSubTotal() {
      return (this.gsecSubTotal = Number(this.g0301) + Number(this.g0304) + Number(this.g0337) + Number(this.g2909));
    },
    tyearSubTotal() {
      return (this.gsecSubTotal = (parseFloat(this.m0301) + parseFloat(this.m0304) + parseFloat(this.m0337) + parseFloat(this.m2909))).toFixed(7);
    },
  },
};
</script>
