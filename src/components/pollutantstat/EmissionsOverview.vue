    ...
    <d-card-body class="pt-0">
      <d-row class="border-bottom py-2 bg-light">
        ...
        <d-col col sm="6">
          ...
          >Посмотреть полный отчет &rarr;</d-button>
        </d-col>
      </d-row>
...
const defaultChartData = {
  ...
  datasets: [
    ...
    {
      label: 'Аналогичный квартал прошлого года',
      fill: 'start',
      data: [
        380,
        430,
...
export default {
  name: 'emissions-overview',
  props: {
    title: {
      type: String,
      default: 'Сравнительные выбросы за период',
    },
    chartData: {
      type: Object,
      default() {
  ...
  mounted() {
    ...
    const YearlyEmissionsOverview = new Chart(this.$refs.canvas, {
      type: 'LineWithLine',
      data: this.chartData,
    ...
    const buoMeta = YearlyEmissionsOverview.getDatasetMeta(0);
    buoMeta.data[0]._model.radius = 0;
    buoMeta.data[this.chartData.datasets[0].data.length - 1]._model.radius = 0;

    // Render the chart.
    YearlyEmissionsOverview.render();
  },
};
</script>
