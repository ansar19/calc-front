  ...
  <d-card class="card-small mb-3 ml-2 mr-2">
    ...
    <d-card-body class="card-body p-2 o-auto" style="height: 25rem;">
      ...
      <d-input-group class="mb-2">
        <d-form-input placeholder="Название промплощадки" type="text" v-model="newWorkSite"/>
        <d-input-group-addon append>
          <d-button class="btn-primary px-2" v-on:click.stop.prevent="addWorkSite">
      ...
      <d-list-group flush class="list-group-small">
        <h5 class="title mt-2">
          <small class="text-muted">Список ПП:</small>
          {{ newWorkSite }}
        </h5>
        <d-list-group-item v-for="workSite in workSiteList" :key="workSite.id" class="d-flex px-3">
          ...
          <span class="ml-auto text-right">
            <d-button class="btn-danger" v-on:click="removeWorkSite(workSite)">
...
export default {
  ...
  props: {
    ...
    title: {
      type: String,
      default: 'Добавление промплощадки',
    },
    workSiteList: {
      type: Array,
      default: () => [
